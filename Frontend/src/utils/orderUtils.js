...
export const calculateArea = (width, height, unit, quantity = 0, allowances = {}) => {
    ...
    const totalWidthInInches = widthInInches + 
    ...
    const materialUsage = (totalWidthInInches * totalHeightInInches / 144) * quantity;
...
export const calculateAmount = (price, discount, quantity) => {
    ...
    const discountAmount = parseFloat(price) * parseFloat(quantity) * (parseFloat(discount || 0) / 100);
    return parseFloat((parseFloat(price) * parseFloat(quantity) - discountAmount).toFixed(2));
};

export const calculateTotals = (orderDetails, discAmount = 0, percentDisc = 0) => {
    const subtotal = orderDetails.reduce((sum, detail) => sum + parseFloat(detail.amount || 0), 0);
    const totalDiscount = parseFloat(discAmount) + (subtotal * (parseFloat(percentDisc) / 100));
...
export const calculatePrintHrs = (squareFeet, quantity, material, materials) => {
    ...
    return parseFloat(((parseFloat(squareFeet) * parseFloat(quantity)) / parseFloat(materialObj.SqFtPerHour)).toFixed(2));
};
